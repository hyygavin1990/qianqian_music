                                ...
                                <div class="ibox-content" style="display: block;">
                                    ...
                                    <font style="vertical-align: inherit;">客户手机号：${calls.phone!}</font>&nbsp;&nbsp;&nbsp;&nbsp;
                                        <font style="vertical-align: inherit;">时间：${callsTime?string("yyyy-MM-dd HH:mm:ss")}</font>&nbsp;&nbsp;&nbsp;&nbsp;
                                            <font style="vertical-align: inherit;">坐席:${staff.nick!} </font>&nbsp;&nbsp;&nbsp;&nbsp;
                                    ...
                                    <button class="control-auth btn btn-success" data-auth="quality_recombine" id="recombine" onclick="recombine()" type="button"  controls="controls"  style="margin:10px 10px 5px 15px;vertical-align: middle;">录音重新生成</button>
                                </div>
                                <div class="ibox-content" style="display: block;">
                        ...
                        <div class="row">
                            <div class="ibox-content" style="display: block; height:698px;">
                                ...
                                <div class="leadsInforListsBox" style="text-align: center">
                                    ...
                                    <#--<input value="${groupId}" type="hidden" id="staffGroupId">-->
                                    ...
                                    <input value="${thirty}" type="hidden" id="thirty">
                                    <input value="${time}" type="hidden" id="date">
                                    <input value="${seat}" type="hidden" id="seat">
                                    <input value="${endState}" type="hidden" id="endState">
                                    <#if nextPhoneId==''>
...
<script type="text/javascript">
    $(document).ready(function(){
        refreshPermission("quality");
        <#--var province = '${phoneArea.province!}';-->
        <#--var city = '${phoneArea.city!}';-->
        <#--setTimeout(function () {-->
            <#--$("#leadsContent").find("select[name='province']").val(province).trigger("change");-->
            <#--$("#leadsContent").find("select[name='city']").val(city);-->
        <#--},1000);-->
    });
    ...
    function recombine(){
        ...
        $.ajax({
            url: "/quality/recombine?phoneId=" + GetQueryString("phoneId"),
            type: 'GET',
            dataType: "json",
            success: function (r) {
                if(r.code ===0){
    ...
    var zhijian = function () {
        window.location.href = "/quality/detailComm?phoneId="+$("#nextPhoneId").val()+"&batchId="+ $("#batchId").val()+"&companyId="+$("#companyId").val()
                +"&phone="+$("#phone").val()+"&projectId="+$("#projectId").val()+
                "&status="+$("#status").val()+"&thirty="+$("#thirty").val()+
                "&time="+$("#date").val()+"&seat="+$("#seat").val()+"&endState="+$("#endState").val();
    };
...
//        formData.append("transferuucallid", $("#transferuucallid").val());
//        formData.append("callId", '$calls.id');
//        formData.append("wav", $("#file")[0].files[0]);
//        var $uploadBtn = $("#upload");
//        $($uploadBtn).attr("disabled", "disabled");
//        $.ajax({
//            url: '/ai/quality/upload',
//            type: 'POST',
//            data: formData,
//            dataType: "json",
//            async: true,
