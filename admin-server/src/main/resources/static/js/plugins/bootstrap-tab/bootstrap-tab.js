...
(function ($, window, document, undefined) {
    ...
    BaseTab.prototype.template = {
        ...
        ul_li_close:'',
        div_content: '<div  class="tab-content" style="border: 0px;"></div>',
    ...
    //新增一个tab页
    BaseTab.prototype.addTab=function (obj,callback) {
        var self=this;
        //nav-tab
        ...
        $("#" + obj.id).load(obj.url,obj.paramter,callback);
        this.stateObj[obj.id] = true;

        if(obj.closeable){
            this.$element.find(".nav-tabs li a[href='#" + obj.id + "'] i.closeable").click(function () {
                var href = $(this).parents("a").attr("href").substring(1);
                ids.splice(ids.indexOf(href),1);
                if(self.getCurrentTabId()==href){
                    //self.$element.find(".nav-tabs li:eq(0) a").tab("show");
                    self.$element.find(".nav-tabs a[href='#" + ids[0] + "']").tab("show");
                    $("#"+ids[0]).addClass("active in");
                }
                $(this).parents("li").remove();
                $("#" + href).remove();
            })
